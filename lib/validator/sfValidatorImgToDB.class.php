...
class sfValidatorImgToDB extends sfValidatorBase
{
  ...
  protected function doClean($value)
  {
    ...
    if ($this->hasOption('resolution')) {
      ...
      if (is_int($resolution[0]) && is_int($resolution[1]))
      {
        $width = $resolution[0];
        $height = $resolution [1];
      }
    ...
    return ($this->catchImage($value['tmp_name'], $width, $height));    
  ...
  public function resizeImage($image, $width, $height) {
    ...
    if($ratio>(4/3)) {
      ...
      $new_width=($height*($old_width/$old_height));
      $new_height=$height;
    ...
    imagecopyresampled($resized_img, $image, $x_offset, $y_offset, 0, 0, $new_width, $new_height, $old_width, $old_height);
    ob_start();
